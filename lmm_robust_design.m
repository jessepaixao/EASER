...
sol_rob_vec=zeros(length(e_vec),3);
...
for i=1:length(e_vec)
    ...
    ks_rnd=unifrnd(k1-0.4*(1-e)*k1,k1+0.4*(1-e)*k1,[1,Nm]);
    ...
    problem.x0 = [5e3 500 5];
    problem.lb = [0 0 0];
    problem.nonlcon = @(q)cnstFcn_abs_damp_mod(q,m1,ks_rnd,c1,m2,omega);
...
Nm=10000;
% sol=sol_rob_vec(1,:)
sol=[3.195533087875585e+03; 1.627687216939067e+03; 1.988096212764022];
% e=0.01;

k_dist(i)=unifrnd(0.6*k1,1.4*k1,[1,Nm]);

for i=1:Nm   
    h_rob(i,:)=abs(recept2MCK(omega,m1,k_dist(i),c1,m2,sol(1),sol(2)));
end

Hc_rob=abs(recept2MCK(omega,m1,k1,c1,m2,sol(1),sol(2)));
...
figure()
set(gcf,'units','normalized','outerposition',[0 0 1 1])
plot(e_vec*100,sol_rob_vec(:,3)*1e-4,'o-','linewidth',2,'color',co(1,:),'markerfacecolor',co(1,:))
set(gca,'FontSize',28,'TickLabelInterpreter','latex')
